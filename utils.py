...
class Player():
    def __init__(self, x, y, name, max_hp, strength, potions, images):
        ...
        temp_list = []
        for i in range(3):  # Supondo que tenha 3 quadros de dano
            img = pygame.image.load(f'img/{self.name}/Hurt/{i}.png').convert_alpha()
            img = pygame.transform.scale(img, (img.get_width()*3, img.get_height()*3))
            temp_list.append(img)
        self.animation_list.append(temp_list)

        # Carregar as animações Dead
        temp_list = []
        for i in range(4):  # Supondo que tenha 4 quadros de morte
    ...
    def atacar(self, alvo):
        if self.alive:
            self.action = 1
            self.frame_index = 0
            self.update()  # Atualizar a animação
            dano = self.strength
            alvo.hp -= dano
            if alvo.hp <= 0:
                alvo.hp = 0
